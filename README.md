...
The first time you open a `bash` shell, `ssh-agent-ctl` will be called, will detect it is running inside a terminal with `stdin` attached, and will then auto-add any private keys sitting in `~/.ssh/private_keys`, which will (for any key with a passphrase) stop and ask you to type in the passphrase. Once you've done this, your instance of `ssh-agent` will remain running (and will hold the added keys resident in memory) until the last logged-in instance of your user account logs out of the system.
